...
<div className="min-h-screen">
  ...
  <div className='mx-auto mt-10 cursor-default flex flex-col justify-between w-full sm:w-1/2 min-h-[400px] h-fit border-[1.5px] border-transparent hover:border-[#333] hover:shadow-[#333] shadow-md py-6 rounded-lg bg-neutral-100'>
    
    <div className={'text-center w-fit mx-auto flex flex-col justify-between gap-10 p-5 rounded-xl ' + titleFont.className}>
      <p className='text-[35px] sm:text-[50px] text-zinc-700'>
        ¡Convertite en un Mecenas!
        <br />
        <span className='text-lg sm:text-3xl'>Ayudá a Recipify a seguir con vida</span>
      </p>
    </div>
    
    <p className='text-[16px] sm:text-[18px] text-center pb-10'>
      También podés acceder a una suscripción <Link href={"/premium"} className='text-sky-700'>haciendo click acá</Link>.
    </p>
    
    <div className='flex flex-col sm:flex-row justify-evenly items-center px-5'>
      <p className={"text-xl text-orange-600 " + titleFont.className}>Donación Anónima</p>
      <p className={"text-2xl sm:text-3xl " + titleFont.className}>$1000</p>
    </div>
    
    <p className={'font-sans text-lg mt-3 text-center px-5 hyphens-auto text-green-400 ' + titleFont.className}>
      ¡Tu donación nos ayuda a seguir en línea!
    </p>
    
    <form onSubmit={handleSubmit} className="text-xl mt-3 mx-auto text-center bg-amber-400 rounded-xl px-2 py-2 w-3/4 sm:w-1/2">
      ...
      <button type='submit' className='w-full py-2 text-white'>¡Doná Ahora!</button>
    </form>
